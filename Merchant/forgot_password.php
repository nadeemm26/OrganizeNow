...
if (isset($_POST['send_otp'])) {
    ...
    $stmt = $conn->prepare("UPDATE merchant SET otp=?, otp_expiry=? WHERE email=?");
...
if (isset($_POST['verify_otp'])) {
    ...
    $stmt = $conn->prepare("SELECT otp, otp_expiry FROM merchant WHERE email=?");
...
if (isset($_POST['reset_password']) && isset($_SESSION['otp_verified'])) {
    ...
    $stmt = $conn->prepare("UPDATE merchant SET password=?, otp=NULL, otp_expiry=NULL WHERE email=?");
    $stmt->bind_param("ss", $new_password, $email);
    
    if ($stmt->execute()) {
        session_destroy();
        echo "<script>alert('✅ Password Reset Successfully!'); window.location.href = '../merchant_login.php';</script>";
        exit();
    } else {
